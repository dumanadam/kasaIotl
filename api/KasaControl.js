...
export default class KasaControl {
  constructor() {
  ...
  login(cloudUserName, cloudPassword) {
    this.uuid = 'e1776657-d638-4211-9dc9-471820b7c6fb';
    this.url = this.urlBase;
    return this.kasa('login', {
  ...
  ```
     */
  info(deviceId) {
    return this.send(deviceId, {system: {get_sysinfo: {}}}).then(
      r => r.system.get_sysinfo,
    );
  }
  ...
  power(deviceId, powerState, transition, options) {
    return this.info(deviceId).then(info => {
      if (typeof info.relay_state !== 'undefined') {
        ...
        return this.send(deviceId, {
          ...
          r =>
            r['smartlife.iot.smartbulb.lightingservice'][
              'transition_light_state'
            ],
        );
      }
